...
import static bb.lib.testing.TestFiles.writeToTempFile;
import static bb.lib.testing.asserts.AssertCliExecutionResult.assertExecution;
...
public class ReturnsDsl {
    ...
    public void verifyOutputIsValidJson() {
        builderWithProjectDefaults().create().toJsonTree(cliRunResult.stdOut());
    }
...
//        assertThat(accountReturns.getError().getIds()).contains(UserErrorId.valueOf(error));
    ...
    private List<String> writeToTempFiles(String[] ledgers) {
        return Stream.of(ledgers)
                ...
                .map(content -> writeToTempFile(content).toString())
                .toList();
    }
    ...
    public ReturnsDsl verifyNonZeroExitCode() {
        assertExecution(cliRunResult).hasNonZeroStatus();
        return this;
    }

    public ReturnsDsl verifyUsagePrinted() {
        assertExecution(cliRunResult).hasPrintedUsage();
        return this;
    }
